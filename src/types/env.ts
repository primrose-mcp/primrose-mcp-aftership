 ...
 * - X-AfterShip-Base-URL: (Optional) Override the default AfterShip API base URL
 */
...
export interface TenantCredentials {
  /** API Key for AfterShip authentication (from X-AfterShip-API-Key header) */
  apiKey?: string;

  /** Override AfterShip API base URL (from X-AfterShip-Base-URL header) */
  baseUrl?: string;
}
...
export function parseTenantCredentials(request: Request): TenantCredentials {
  ...
  return {
    apiKey: headers.get('X-AfterShip-API-Key') || undefined,
    baseUrl: headers.get('X-AfterShip-Base-URL') || undefined,
  };
}
...
export function validateCredentials(credentials: TenantCredentials): void {
  if (!credentials.apiKey) {
    throw new Error('Missing credentials. Provide X-AfterShip-API-Key header.');
  }
}
